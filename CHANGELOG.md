...
### Changed
- Literature-review and citation-audit prompts retrieve source passages before judging claims, report available locators and evidence status, and distinguish abstract-only summaries from verified passages.
- Setup and configuration guidance explains embedding dependencies, PDF indexing defaults, search coverage, privacy data paths and the single-library index limit.
- Added first-research-task, search-readiness, missing-PDF and annotations-to-literature-notes guides, with current platform download guidance.
