...
| `ZOTEUS_ZOTERO_DEADLINE_MS` | `25000` | Per-request time budget for reads against the **desktop app**, in milliseconds. The budget exists so a stuck read fails with an actionable message instead of hanging until your MCP client's own per-call timeout, and 25 s is right for a desktop API that normally answers a listing in under a second. Raise it when yours does not: on a large library some machines answer an attachment listing slowly enough that an index build's attachment map aborts partway through, and a build over a partial map indexes only the attachments the map reached. Accepted between `5000` and `600000`; anything outside that is ignored with a warning and the 25 s default stands. Scoped to the desktop app's listings and item reads, so raising it never lets a cloud Web API call hang that long. It does not change the liveness probe (1.5 s), file uploads to the desktop app (which pass their own, longer budgets), or the download of an attachment's bytes from it, which passes none and so stays on the 25 s default: that request is a redirect the app answers at once followed by bytes read off disk, not a query it has to compute. |
...
| `ZOTEUS_EMBEDDINGS` | `local` | Semantic-search embeddings provider (`local` model, `openai`, `gemini`, `ollama`, or `off`). `local` and `ollama` both keep the text on your machine; `openai` and `gemini` send it to that provider. See [`ollama.md`](./ollama.md). |
| `ZOTEUS_OLLAMA_URL` | `http://127.0.0.1:11434` | Base URL of the Ollama daemon used when `ZOTEUS_EMBEDDINGS=ollama`. The default model is `all-minilm` (`ollama pull all-minilm`). `nomic-embed-text` needs task prefixes Zoteus does not send and retrieves worse without them, silently, so Zoteus warns when you select it. |
...
| `ZOTEUS_OCR` | `off` | Read PDFs that have no text layer by optical character recognition. `auto` uses an OCR engine when one resolves and says exactly what to install when none does. The engine is an optional runtime dependency this package does not ship, for the same reason the embedding runtime is not bundled. See [`ocr.md`](./ocr.md). |
| `ZOTEUS_OCR_PATH` | — | Where to resolve the OCR engine from when the install cannot see it itself; the same escape hatch `ZOTEUS_TRANSFORMERS_PATH` is for the embedding runtime. |
| `ZOTEUS_OCR_MAX_PAGES` | `8` | Pages one OCR call may read before it stops and tells the caller to ask for the next range. OCR is orders of magnitude slower than reading a text layer, so an unbounded call looks like a hang to an MCP client. |
| `ZOTEUS_OCR_LANGS` | `eng` | Tesseract language codes, `+` separated (e.g. `eng+deu`). Each is a separate trained-data file the engine must have locally. |
...
| `ZOTEUS_INDEX_MAX_OPEN` | `4` | How many library indexes one server keeps open at once. A data directory holds one index file per library, and each open index costs a SQLite handle and its cached pages, so the least recently used is saved and closed when this is exceeded. It bounds memory and file handles, not how many libraries you may index. See [`multiple-library-indexes.md`](./multiple-library-indexes.md). |
...
| `ZOTEUS_OA_FETCH` | `true` | Whether open-access discovery may DOWNLOAD the copy it finds. Finding and reporting an OA link is always allowed; set this false on a shared deployment to stop Zoteus fetching bytes from whatever host the discovery named. Discovery still reports the link, so nothing goes silent. See [`open-access-pdfs.md`](./open-access-pdfs.md). |
| `ZOTEUS_IMPORT_MAX_ENTRIES` | `200` | Entries accepted from one BibTeX/RIS/CSL-JSON import before it refuses and names the cap, rather than starting a bulk write. See [`importing-bibliographies.md`](./importing-bibliographies.md). |
...
| `ZOTEUS_LOG_FILE` | unset | A file every log line is appended to as well as stderr, in the same format. For a server nobody's terminal is attached to (a Windows scheduled task, a service manager that discards stderr), it is the only record of what the process was doing. A file that cannot be written is reported once on stderr and never stops the server from starting. |
